...
public class ReminderService {
    ...
    @Autowired
    public ReminderService(EmailService emailService, UserService userService, NoteRepository noteRepository, ReminderCheckRepository reminderCheckRepository, SimpMessagingTemplate template){
    ...
    //Zwróć wszystkie sprawdzenia dla danego użytkownika
    public List<ReminderCheck> getAllReminderChecksByUserId(int id){
    ...
    public void reminder() throws MessagingException {
        ...
        for(Note note : notes){
            ...
            for(User user : users){
                ...
                ReminderCheck reminderCheck = reminderCheckRepository.findByUserIdAndNoteId(user.getId().intValue(),note.getId().intValue());
                if(!reminderCheck.isReminded() && !duration.isNegative() && duration.toMinutes()<reminderCheck.getReminderTime()){
